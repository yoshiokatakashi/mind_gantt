...
1. 技術スタック最終決定（**Supabase を採用**）
2. GitHubリポジトリ作成（README 初期版作成済）・Vercel連携（進行中）・CIテンプレート追加（GitHub Actions による自動ビルド用 ci.yml を追加）
3. プロトタイプ（Week1）：Next.js 起動環境構築＋react-flow によるマインドマップ初期画面を実装し、社内でフィードバック取得
